    ...
    public static class SettingsApp
    {
        ...
        public static void OpenMainPage(BaseKernelConfig? settingsType, bool useSelection = false)
        {
            ...
            while (!PromptFinished)
            {
                ...
                {
                    new($"{MaxSections + 1}", LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_FINDOPTION"), LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_FINDOPTION_DESC")),
                    new($"{MaxSections + 2}", LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SAVE"), LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SAVE_DESC")),
                    ...
                    new($"{MaxSections + 4}", LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_LOAD"), LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_LOAD_DESC")),
                    ...
                    new($"{MaxSections + 8}", LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_MIGRATE"), LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_MIGRATE_DESC")),
                ...
                string finalTitle = LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_TITLE");
                int Answer = SelectionStyle.PromptSelection(RenderHeader(finalTitle, TextTools.FormatString(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_HEADER"), settingsType.GetType().Name)),
                    sections, altSections);
                if (Answer >= 0 & Answer < MaxSections)
                {
                    // The selected answer is a section
                    InfoBoxNonModalColor.WriteInfoBox(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SECTION_LOADING"));
                ...
                else if (Answer == MaxSections + 7)
                {
                    ...
                    if (!ConfigMigration.MigrateAllConfig())
                        InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_MIGRATEOLDCONFIG_SUCCESS") + " " +
                            LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_GOBACK"), new InfoBoxSettings()
                            {
                ...
                else
                {
                    ...
                    InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_OPTIONINVALID") + " " + LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_GOBACK"), new InfoBoxSettings()
                    {
                        ForegroundColor = KernelColorTools.GetColor(KernelColorType.Error)
                    }, Answer);
                }
        ...
        public static void OpenSection(string Section, SettingsEntry SettingsSection, BaseKernelConfig settingsType)
        {
            ...
            try
            {
                ...
                while (!SectionFinished)
                {
                    ...
                    if (hasUnsupportedConfigs)
                        Notes = LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_UNSUPPORTEDCONFIGS") + $" {string.Join(", ", unsupportedConfigNames)}";
                    ...
                    else
                    {
                        ...
                        TextWriters.Write(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_GOBACK"), true, KernelColorType.Error);
                        Input.ReadKey();
                    }
                }
            }
            ...
            {
                SettingsAppTools.HandleError(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SECTIONINVALID"), ex);
            }
        }
        ...
        public static void OpenKey(int KeyNumber, SettingsEntry SettingsSection, BaseKernelConfig settingsType)
        {
            ...
            {
                SettingsAppTools.HandleError(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_KEYINVALID"), ex);
            }
        }
        ...
        public static void VariableFinder(BaseKernelConfig configType)
        {
            try
            {
                ...
                string SearchFor = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SEARCH_PROMPT"));

                // Search for the setting
                ConsoleWrapper.CursorVisible = false;
                InfoBoxNonModalColor.WriteInfoBox(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SEARCH_SEARCHING"));
                ...
                if (Results.Count > 0)
                {
                    ...
                    {
                        // Prompt for setting
                        sel = InfoBoxSelectionColor.WriteInfoBoxSelection([.. finalResults], LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SEARCH_SELECT"));
                        ...
                        var Section = configType.SettingsEntries?[SectionIndex] ??
                            throw new KernelException(KernelExceptionType.Config, LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SEARCH_EXCEPTION_NOSECTION"));
                ...
                {
                    InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SEARCH_NORESULTS"), new InfoBoxSettings()
                    {
                        ForegroundColor = KernelColorTools.GetColor(KernelColorType.Error)
                    });
                }
            ...
            {
                InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_SEARCH_FAILED") + $" {ex.Message}", new InfoBoxSettings()
                {
                    ForegroundColor = KernelColorTools.GetColor(KernelColorType.Error)
                });
            }
