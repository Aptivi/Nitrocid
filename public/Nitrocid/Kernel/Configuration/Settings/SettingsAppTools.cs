    ...
    internal static class SettingsAppTools
    {
        ...
        internal static void HandleError(string message, Exception? ex = null)
        {
            if (ex is null)
            {
                ...
                string finalSection = LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_ERROR_TITLE");
                InfoBoxModalColor.WriteInfoBoxModal(
                    $"  * {finalSection}\n\n" +
                    $"{message}\n\n" +
            ...
            else
            {
                ...
                InfoBoxModalColor.WriteInfoBoxModal(
                    $"  * {finalSection}\n\n" +
                    $"{message}\n\n" +
                    $"{LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_ERROR_DESC_EXCEPTION1")}\n" +
                    $"  - {ex.Message}\n\n" +
                    $"{LanguageTools.GetLocalized("NKS_KERNEL_CONFIGURATION_SETTINGS_APP_ERROR_DESC_EXCEPTION2")}", new InfoBoxSettings()
                    {
                        ForegroundColor = KernelColorTools.GetColor(KernelColorType.Error)
                    }
                );
