            ...
            try
            {
                ...
                if (ConfigTools.IsCustomSettingBuiltin(typeName))
                    baseConfigurations[typeName] = (BaseKernelConfig?)JsonConvert.DeserializeObject(jsonContents, type.GetType()) ??
                        throw new KernelException(KernelExceptionType.Config, LanguageTools.GetLocalized($"Can't deserialize the base configuration {typeName}"));
                else
                    customConfigurations[typeName] = (BaseKernelConfig?)JsonConvert.DeserializeObject(jsonContents, type.GetType()) ??
                        throw new KernelException(KernelExceptionType.Config, LanguageTools.GetLocalized($"Can't deserialize the custom configuration {typeName}"));
            }
            catch (Exception e)
            {
