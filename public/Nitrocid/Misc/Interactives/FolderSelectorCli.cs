        ...
        internal void Select(FileSystemEntry? currentFileSystemEntry)
        {
            try
            {
                ...
                if (currentFileSystemEntry.Type == FileSystemEntryType.Directory)
                {
                    ...
                    InfoBoxModalColor.WriteInfoBoxModal(Translate.DoTranslation("Selected '{0}'. Press ESC to exit and confirm your selection."), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor }, selectedFolder);
                }
            }
            catch (Exception ex)
            {
