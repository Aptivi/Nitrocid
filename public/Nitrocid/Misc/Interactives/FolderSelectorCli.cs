        ...
        internal void Select(FileSystemEntry? currentFileSystemEntry)
        {
            try
            {
                ...
                if (currentFileSystemEntry.Type == FileSystemEntryType.Directory)
                {
                    ...
                    InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_MISC_INTERACTIVES_FMTUI_SELECTSUCCESS"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor }, selectedFolder);
                }
            }
            catch (Exception ex)
            {
