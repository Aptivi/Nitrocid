...
namespace Nitrocid.ShellPacks.Shells.Archive.Presets
{
    ...
    public class ArchivePowerLineBG3Preset : PromptPresetBase, IPromptPreset
    {
        ...
        private string PresetPromptBuilder()
        {
            ...
            [
                new PowerLineSegment(new Color(255, 255, 85), new Color(25, 25, 25), Path.GetFileName(archiveShell.FileStream?.Name) ?? "", default, TransitionPartChar),
                new PowerLineSegment(new Color(255, 255, 85), new Color(25, 25, 25), archiveShell.CurrentArchiveDirectory ?? "", default, TransitionPartChar)
            ];
