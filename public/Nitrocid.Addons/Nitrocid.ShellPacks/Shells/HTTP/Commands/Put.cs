...
namespace Nitrocid.ShellPacks.Shells.HTTP.Commands
{
    class PutCommand : BaseCommand, ICommand
    {
        ...
        public override string HelpDefinition =>
            LanguageTools.GetLocalized("NKS_SHELLPACKS_HTTP_COMMAND_PUT_DESC");
        ...
        public override int Execute(IShell? shell, CommandParameters parameters, ref string variableValue)
        {
            ...
            // Print a message
            TextWriterColor.Write(LanguageTools.GetLocalized("NKS_SHELLPACKS_COMMON_FS_UPLOADINGFILE"), true, ThemeColorType.Progress, parameters.ArgumentsList[1]);

            try
            {
                var ResponseTask = httpShell.HttpPutFile(parameters.ArgumentsList[0], parameters.ArgumentsList[1]);
                ResponseTask.Wait();
