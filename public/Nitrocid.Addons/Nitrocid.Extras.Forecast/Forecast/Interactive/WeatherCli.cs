...
namespace Nitrocid.Extras.Forecast.Forecast.Interactive
{
    ...
    public class WeatherCli : BaseInteractiveTui<(double, double)>, IInteractiveTui<(double, double)>
    {
        ...
        public override string GetInfoFromItem((double, double) item)
        {
            ...
            CheckApiKey();
            InfoBoxNonModalColor.WriteInfoBox(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_LOADING") + $" {item.Item1}, {item.Item2}...", new InfoBoxSettings()
            {
        ...
        internal void Add()
        {
            ...
            string cityName = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_CITYNAMEPROMPT"), new InfoBoxSettings()
            {
                ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
            });
            ...
            if (cities.Count == 0)
            {
                InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_NOCITIES"), new InfoBoxSettings()
                {
                    ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                    BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
                });
            ...
            int cityIdx = InfoBoxSelectionColor.WriteInfoBoxSelection(cities.Select((kvp, idx) => new InputChoiceInfo($"{idx + 1}", $"[{kvp.Value.Item1}, {kvp.Value.Item2}] {kvp.Key}")).ToArray(), LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_SELECTCITY"), new InfoBoxSettings()
            {
                ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
            });
        ...
        internal void AddManually()
        {
            ...
            string latString = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_LATPROMPT"), new InfoBoxSettings()
            {
                ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
            });
            string lngString = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_LONPROMPT"), new InfoBoxSettings()
            {
                ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
            });
            ...
            {
                InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_LATINVALID"), new InfoBoxSettings()
                {
                    ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                    BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
                });
            ...
            {
                InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_LONINVALID"), new InfoBoxSettings()
                {
                    ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                    BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
                });
        ...
        internal void CheckApiKey()
        {
            if (string.IsNullOrEmpty(Forecast.ApiKey))
            {
                do
                {
                    Forecast.ApiKey = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_APIKEYPROMPT"), new InfoBoxSettings()
                    {
                        ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                        BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
                    }, InfoBoxInputType.Password);
                    if (string.IsNullOrEmpty(Forecast.ApiKey))
                        InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_FORECAST_WEATHER_TUI_APIKEYNOTPROVIDED"), new InfoBoxSettings()
                        {
                            ForegroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxForeground),
                            BackgroundColor = KernelColorTools.GetColor(KernelColorType.TuiBoxBackground),
                        });
