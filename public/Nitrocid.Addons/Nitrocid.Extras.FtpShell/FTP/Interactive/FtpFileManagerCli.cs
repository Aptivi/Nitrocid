...
namespace Nitrocid.Extras.FtpShell.FTP.Interactive
{
    internal class FtpFileManagerCli : BaseInteractiveTui<FileSystemEntry, FtpListItem>, IInteractiveTui<FileSystemEntry, FtpListItem>
    {
        ...
        internal void CopyTo(FileSystemEntry? entry1, FtpListItem? entry2)
        {
            ...
            try
            {
                string path = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_COPYPROMPT"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                ...
                if (CurrentPane == 2)
                {
                    ...
                    if (FTPFilesystem.FTPExists(path))
                    {
                        ...
                        InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_FILENOTFOUND"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                }
                else
                {
                    // We are dealing with the local side.
                    ...
                    if (FilesystemTools.FolderExists(path))
                    {
                        if (FilesystemTools.TryParsePath(path))
                        {
                            ...
                            InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_NEEDSCORRECTPATH"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                    }
                    else
                        InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_FILENOTFOUND"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                }
        ...
        internal void MoveTo(FileSystemEntry? entry1, FtpListItem? entry2)
        {
            ...
            try
            {
                string path = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_MOVEPROMPT"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
        ...
        internal void Rename(FileSystemEntry? entry1, FtpListItem? entry2)
        {
            ...
            try
            {
                string filename = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_NEWFILENAMEPROMPT"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                ...
                if (CurrentPane == 2)
                {
                    ...
                    if (!FTPFilesystem.FTPExists(currentEntry.FullName.RemoveSuffix(currentEntry.Name) + $"/{filename}"))
                    {
                        ...
                        InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_FILEFOUND"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                }
                else
                {
                    // We are dealing with the local side.
                    ...
                    if (!FilesystemTools.FileExists(filename))
                    {
                        if (FilesystemTools.TryParseFileName(filename))
                        {
                            ...
                            InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_FILENAMEINVALID"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                    }
                    else
                        InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_FILEFOUND"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
                }
        ...
        {
            string path = InfoBoxInputColor.WriteInfoBoxInput(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_NEWDIRNAMEPROMPT"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
            ...
            if (CurrentPane == 2)
            {
                ...
                if (!FTPFilesystem.FTPExists(path))
                {
                    ...
                    InfoBoxModalColor.WriteInfoBoxModal(LanguageTools.GetLocalized("NKS_SHELLPACKS_FTPSFTP_FMCLI_FOLDERFOUND"), new InfoBoxSettings() { ForegroundColor = Settings.BoxForegroundColor, BackgroundColor = Settings.BoxBackgroundColor });
            }
            else
            {
                // We are dealing with the local side.
