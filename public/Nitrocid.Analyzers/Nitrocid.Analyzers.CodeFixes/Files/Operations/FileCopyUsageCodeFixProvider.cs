...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(FileCopyUsageCodeFixProvider)), Shared]
    public class FileCopyUsageCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(FileCopyUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.FileCopyUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.FileCopyUsageCodeFixTitle)),
                diagnostic);
        }
