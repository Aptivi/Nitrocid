...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(FileExistsUsageCodeFixProvider)), Shared]
    public class FileExistsUsageCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(FileExistsUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.FileExistsUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.FileExistsUsageCodeFixTitle)),
                diagnostic);
        }
