...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PathExistsUsageCodeFixProvider)), Shared]
    public class PathExistsUsageCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(PathExistsUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.PathExistsUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.PathExistsUsageCodeFixTitle)),
                diagnostic);
        }
        ...
        private async Task<Solution> UseTextToolsFormatStringAsync(Document document, MemberAccessExpressionSyntax typeDecl, CancellationToken cancellationToken)
        {
            if (typeDecl.Expression is IdentifierNameSyntax identifier)
            {
                ...
                // We need to have a syntax that calls FilesystemTools.Exists
                var classSyntax = SyntaxFactory.IdentifierName("FilesystemTools");
                var methodSyntax = SyntaxFactory.IdentifierName("Exists");
