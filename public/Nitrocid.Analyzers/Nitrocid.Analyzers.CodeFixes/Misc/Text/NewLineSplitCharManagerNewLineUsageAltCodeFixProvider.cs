...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(NewLineSplitCharManagerNewLineUsageAltCodeFixProvider)), Shared]
    public class NewLineSplitCharManagerNewLineUsageAltCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(NewLineSplitCharManagerNewLineUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.NewLineSplitCharManagerNewLineUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.NewLineSplitCharManagerNewLineUsageCodeFixTitle)),
                diagnostic);
        }
        ...
        {
            // We need to have a syntax that calls myvar.SplitNewLinesOld
            var maes = (MemberAccessExpressionSyntax)typeDecl.Expression;
            var varName = maes.Expression;
            var classSyntax = varName;
            var methodSyntax = SyntaxFactory.IdentifierName("SplitNewLinesOld");
