...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(KernelDateTimeToDateStringUtcUsageCodeFixProvider)), Shared]
    public class KernelDateTimeToDateStringUtcUsageCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(KernelDateTimeToDateStringUtcUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.KernelDateTimeToDateStringUtcUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.KernelDateTimeToDateStringUtcUsageCodeFixTitle)),
                diagnostic);
        }
        ...
        private async Task<Solution> UseTextToolsFormatStringAsync(Document document, MemberAccessExpressionSyntax typeDecl, CancellationToken cancellationToken)
        {
            if (typeDecl.Expression is IdentifierNameSyntax identifier)
            {
                ...
                // We need to have a syntax that calls TimeDateRenderersUtc.RenderDateUtc
                var classSyntax = SyntaxFactory.IdentifierName("TimeDateRenderersUtc");
                var methodSyntax = SyntaxFactory.IdentifierName("RenderDateUtc");
