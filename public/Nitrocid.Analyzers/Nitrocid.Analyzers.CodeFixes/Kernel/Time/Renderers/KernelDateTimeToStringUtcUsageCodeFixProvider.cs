...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(KernelDateTimeToStringUtcUsageCodeFixProvider)), Shared]
    public class KernelDateTimeToStringUtcUsageCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(KernelDateTimeToStringUtcUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.KernelDateTimeToStringUtcUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.KernelDateTimeToStringUtcUsageCodeFixTitle)),
                diagnostic);
        }
        ...
        private async Task<Solution> UseTextToolsFormatStringAsync(Document document, MemberAccessExpressionSyntax typeDecl, CancellationToken cancellationToken)
        {
            if (typeDecl.Expression is IdentifierNameSyntax identifier)
            {
                ...
                // We need to have a syntax that calls TimeDateRenderersUtc.RenderUtc
                var classSyntax = SyntaxFactory.IdentifierName("TimeDateRenderersUtc");
                var methodSyntax = SyntaxFactory.IdentifierName("RenderUtc");
