...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(IsOsPlatformWindowsUsageCodeFixProvider)), Shared]
    public class IsOsPlatformWindowsUsageCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(IsOsPlatformWindowsUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.IsOsPlatformWindowsUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.IsOsPlatformWindowsUsageCodeFixTitle)),
                diagnostic);
        }
        ...
        private async Task<Solution> UseTextToolsFormatStringAsync(Document document, MemberAccessExpressionSyntax typeDecl, CancellationToken cancellationToken)
        {
            if (typeDecl.Expression is IdentifierNameSyntax identifier)
            {
                ...
                // We need to have a syntax that calls PlatformHelper.IsOnWindows
