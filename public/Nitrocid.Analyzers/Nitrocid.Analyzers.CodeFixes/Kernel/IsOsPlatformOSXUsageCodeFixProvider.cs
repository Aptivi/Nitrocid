...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(IsOsPlatformOSXUsageCodeFixProvider)), Shared]
    public class IsOsPlatformOSXUsageCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds =>
            ImmutableArray.Create(IsOsPlatformOSXUsageAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.IsOsPlatformOSXUsageCodeFixTitle,
                    createChangedSolution: c => UseTextToolsFormatStringAsync(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.IsOsPlatformOSXUsageCodeFixTitle)),
                diagnostic);
        }
        ...
        private async Task<Solution> UseTextToolsFormatStringAsync(Document document, MemberAccessExpressionSyntax typeDecl, CancellationToken cancellationToken)
        {
            if (typeDecl.Expression is IdentifierNameSyntax identifier)
            {
                ...
                // We need to have a syntax that calls KernelPlatform.IsOnMacOS
                var classSyntax = SyntaxFactory.IdentifierName("KernelPlatform");
                var methodSyntax = SyntaxFactory.IdentifierName("IsOnMacOS");
                ...
                // Actually replace
                var parentSyntax = typeDecl.Parent;
