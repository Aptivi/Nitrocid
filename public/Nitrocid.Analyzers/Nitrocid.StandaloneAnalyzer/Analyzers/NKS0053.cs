...
{
    internal class NKS0053 : IAnalyzer
    {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (exp.Parent is InvocationExpressionSyntax invocation)
                {
                    ...
                    if (argsList.Count == 1)
                    {
                        ...
                        if (argToken.ValueText == "STY" && location is not null)
                        {
                            AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses RuntimeInformation.IsOSPlatform(OSPlatform.OSX) instead of PlatformHelper.IsOnMacOS()");
                            found = true;
                        }
                    }
                }
