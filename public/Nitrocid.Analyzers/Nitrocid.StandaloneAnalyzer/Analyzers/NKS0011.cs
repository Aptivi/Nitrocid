...
{
    internal class NKS0011 : IAnalyzer
    {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Directory))
                    {
                        ...
                        {
                            AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses Directory.GetFileSystemEntries instead of FilesystemTools.GetFileSystemEntries()");
                            found = true;
                        }
                    }
                }
