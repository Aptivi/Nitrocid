...
{
    internal class NKS0050 : IAnalyzer
    {
        public bool Analyze(Document document)
        {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (expMaes.Name is IdentifierNameSyntax identifier)
                {
                    ...
                    {
                        AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses CharManager.NewLine instead of TextTools.SplitNewLines()");
                        found = true;
                    }
                }
            }
