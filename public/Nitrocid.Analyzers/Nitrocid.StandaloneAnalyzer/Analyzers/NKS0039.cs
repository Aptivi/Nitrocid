                    ...
                    if (argsList.Count == 1)
                    {
                        ...
                        if (argSyntax.Expression is IdentifierNameSyntax argIdentifier)
                        {
                            if (argIdentifier.Identifier.Text == nameof(OSPlatform))
                            {
                                ...
                                var idName = name.Identifier.Text;
                                if (idName == nameof(OSPlatform.Windows) && location is not null)
                                {
                                    AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses RuntimeInformation.IsOSPlatform(OSPlatform.Windows) instead of PlatformHelper.IsOnWindows()");
                                    found = true;
                                }
                            }
                        }
