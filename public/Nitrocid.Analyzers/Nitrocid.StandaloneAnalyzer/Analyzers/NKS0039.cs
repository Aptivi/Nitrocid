                    ...
                    if (argsList.Count == 1)
                    {
                        ...
                        if (argSyntax.Expression is IdentifierNameSyntax argIdentifier)
                        {
                            if (argIdentifier.Identifier.Text == nameof(OSPlatform))
                            {
                                ...
                                {
                                    AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses RuntimeInformation.IsOSPlatform(OSPlatform.Windows) instead of KernelPlatform.IsOnWindows()");
                                    found = true;
                                }
                            }
                        }
