...
{
    internal class NKS0033 : IAnalyzer
    {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == "TimeDateTools")
                    {
                        ...
                        {
                            AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses TimeDateTools.KernelDateTimeUtc.ToString instead of TimeDateRenderersUtc.RenderUtc()");
                            found = true;
                        }
                    }
                }
