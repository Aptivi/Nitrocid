...
{
    internal class NKS0055 : IAnalyzer
    {
        public bool Analyze(Document document)
        {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (maes.Name is IdentifierNameSyntax identifier)
                {
                    ...
                    // Let's see if the caller tries to access myvar.SplitNewLinesOld().
                    var idName = identifier.Identifier.Text;
                    if (idName == "SplitNewLinesOld")
                    {
                        AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses TextTools.SplitNewLinesOld() instead of TextTools.SplitNewLines()");
                        found = true;
                    }
                }
            }
        ...
        public async Task SuggestAsync(Document document, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                {
                    // Get the method
                    if (idName.Identifier.Text != "SplitNewLinesOld")
                        continue;
