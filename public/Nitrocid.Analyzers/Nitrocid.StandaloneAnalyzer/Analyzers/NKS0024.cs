...
{
    internal class NKS0024 : IAnalyzer
    {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(TimeZoneInfo))
                    {
                        ...
                        {
                            AnalyzerTools.PrintFromLocation(location, document, GetType(), "Caller uses TimeZoneInfo.Local instead of TimeZones.GetCurrentZoneInfo()");
                            found = true;
                        }
                    }
                }
        ...
        public async Task SuggestAsync(Document document, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (exp.Expression is IdentifierNameSyntax identifier && exp.Name is IdentifierNameSyntax idName)
                {
                    ...
                    if (identifier.Identifier.Text != nameof(TimeZoneInfo))
                        continue;
                    if (idName.Identifier.Text != nameof(TimeZoneInfo.Local))
                        continue;
